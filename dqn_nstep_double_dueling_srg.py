...
#                         env.action_space.n).to(device)
net = dqn_model.DuelingDQN_srg(env.observation_space.shape, env.action_space.n).to(device)
...
with common.RewardTracker(writer, params['stop_reward']) as reward_tracker: #create a reward tracker object
    while True:
        ...
        loss_v, feature_loss, qvalue_loss = common.calc_loss_srg(batch, net, tgt_net.target_model, gamma=params['gamma']**args.nsteps, device=device, double=args.double)
        if frame_idx % 1E3 == 0:
